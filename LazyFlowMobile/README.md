...
The keyboard's **Start talking** button writes a session command and posts a local **Open LazyFlow** notification. Tapping the notification opens the app and replays the pending command. Notification permission is requested after the first session started inside LazyFlow. No URL-opening workaround or implementation-detail API is used.
